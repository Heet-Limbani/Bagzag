...
import com.bagzag.app.databinding.ViewProductCardWrapContentBinding

class AdapterProductCardWrapContent :
    RecyclerView.Adapter<AdapterProductCardWrapContent.ProductCardViewHolder>() {

    val productList = mutableListOf<ProductCard>()

    fun setProductList(list: List<ProductCard>) {
        productList.clear()
        productList.addAll(list)
        notifyDataSetChanged()
    }

    inner class ProductCardViewHolder(val rootView: ViewProductCardWrapContentBinding) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ProductCardViewHolder {
        return ProductCardViewHolder(
            ViewProductCardWrapContentBinding.inflate(
                LayoutInflater.from(
                    parent.context
                ), parent, false
            )
